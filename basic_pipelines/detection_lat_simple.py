...
REQUIRED_STAGES = [
    ...
    "inference_out",    # hailonet:src (after inference)
...
class StageTimer:
    ...
    def compute(self, pts_ns: int):
        ...
        result = {
            ...
            "inference_ms": (rec["inference_out"] - rec["src_out"]) / 1e6,
            "to_cb_ms":   (rec["cb_out"] - rec["inference_out"]) / 1e6,
...
class LatencyStats:
    ...
    def __init__(self):
        self.stages = [
            "upstream_ms", "inference_ms", "to_cb_ms", "e2e_ms", "pts_to_cb_ms"
        ]
...
def attach_stage_probes(user_data):
    """
    Attach probes to the simplified pipeline (no wrapper, no tracker).
    """
    ...
    # After inference
    p = _pad("inference_hailonet", "src")
    if p:
        p.add_probe(Gst.PadProbeType.BUFFER, make_stage_probe(user_data, "inference_out"))
        attached += 1
        print("[PROBES OK] inference_hailonet:src -> inference_out")

    user_data.probes_attached = True
    print(f"[PROBES SUMMARY] attached={attached}/2\n")
        ...
        if user_data.get_count() % 30 == 0:
            if stage is not None:
                ...
                msg = (
                    ...
                    f"inference={stage['inference_ms']:.1f} ms | "
